...
**0.014786 nats** (`ceiling.py` A). After Anamorph reweights, a *fresh* converged
...
ratio, fitted on truth — against Anamorph's own weights on the criterion Anamorph selects
...
| Anamorph          | **−2.322e-4** | 4.579e-3 | 73.3% |

**Anamorph scores better than the truth on the detector-level criterion**, on events
...
deterministic convex solve. Anamorph's pathology reproduces with no adversary, no
...
- **Generator capacity** — solid where it rests on `tilt.py` (§3), which is a
  ...
  reach +77.0%. The *Anamorph* number in that table (+78.9%) is a single run and
...
Anamorph performs at the ceiling its **objective** permits. The remaining gap is a
...
gives Anamorph, fitted on train and scored on val — the same two splits `val_d`
comes from. Anamorph's `d` scoring far above this floor means `d` is the bottleneck;
...
against `x_data`. Scored through `anamorph.train.weighted_bce`, so the number is
...
so it is directly comparable to §2's oracle at 80.1% (13124 of 16384) and Anamorph
...
One run proves nothing: Anamorph is an adversarial min-max game, so two runs at different seeds in the _same_ dtype already differ by ~1 percentage point per dimension. Run an ensemble in each dtype and compare the two distributions. The script runs two ensembles in each dtype, and compares them.
...
`anamorph`. What the quarantine does not settle is the GPU, and that is what this
measures.
