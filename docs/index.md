...
**Reweighting Adversarial Networks (RAN)** is a machine learning framework for high-energy physics (HEP) that learns continuous per-event weights to correct simulated (Monte Carlo) distributions so they match observed detector data.

Built on **Keras 3** with the **JAX** backend, RAN is engineered end-to-end for performance, mathematical precision, and scalable multidimensional unfolding.
...
<figure class="ran-figure" markdown="span">
  ![RAN reweights particle-level events and is scored at detector level](assets/schematic.svg){ .ran-schematic }
  <figcaption>
...
RAN solves this problem through a two-player adversarial game:
...
-   **[Getting Started](getting-started/installation.md)**
    ...
    Install dependencies using `uv`, configure CUDA or Apple Silicon, and train
...
-   **[User Guide](user-guide/cli.md)**
    ...
    Explore the `ran` CLI commands: training, evaluation, automated LaTeX
