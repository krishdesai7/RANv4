...
Compare Anamorph against IBU as the detector response gets more non-linear.
...
Each point is one `anamorph sweep ran` invocation writing one `point_NN.json`, so points run concurrently and independently — a point that crashes leaves a gap rather than sinking the sweep. `collect` then reads whatever landed and joins it on `s_index`. Nothing here imports `anamorph.train` at module scope, so `collect` costs no keras or jax import.
...
### `anamorph.experiments.cubic_sweep::_ibu_point`
...
The fit takes `mc.z`, `mc.x` and `data` — what a real measurement has — and is applied to the same `mc.z` that Anamorph's weights are applied to. `truth` reaches neither method and appears only in the score.
...
```shell
anamorph sweep anamorph --s-index 0 --sweep-dir ...
anamorph sweep collect --sweep-dir ...
```

### `anamorph.experiments.cubic_sweep::make_particles`
...
### `anamorph.experiments.cubic_sweep::_sweep_point`
...
### `anamorph.experiments.cubic_sweep::run_ran`

Run both methods at one sweep point and write point\_{index}.json.

Particle samples are drawn once (fixed `seed`) so only s varies across points. For Anamorph, `z_unfolded` is the `z_gen` sample reweighted by `g(z_gen)`; for IBU it is the same sample reweighted by the binned correction `_ibu_point` fits. The subcommand keeps the name `anamorph` because Anamorph is what it trains — IBU rides along on the populations that training already required.
...
### `anamorph.experiments.cubic_sweep::_complete_points`
...
### `anamorph.experiments.cubic_sweep::collect`
