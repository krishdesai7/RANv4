...
Compare Deconvolve against IBU as the detector response gets more non-linear.
...
Each point is one `deconvolve sweep ran` invocation writing one `point_NN.json`, so points run concurrently and independently — a point that crashes leaves a gap rather than sinking the sweep. `collect` then reads whatever landed and joins it on `s_index`. Nothing here imports `deconvolve.train` at module scope, so `collect` costs no keras or jax import.
...
### `deconvolve.experiments.cubic_sweep::_ibu_point`

Unfold one sweep point with IBU and score it the way Deconvolve is scored.
...
The fit takes `mc.z`, `mc.x` and `data` — what a real measurement has — and is applied to the same `mc.z` that Deconvolve's weights are applied to. `truth` reaches neither method and appears only in the score.
...
```shell
deconvolve sweep deconvolve --s-index 0 --sweep-dir ...
deconvolve sweep collect --sweep-dir ...
```

### `deconvolve.experiments.cubic_sweep::make_particles`
...
### `deconvolve.experiments.cubic_sweep::_sweep_point`
...
### `deconvolve.experiments.cubic_sweep::run_ran`

Run both methods at one sweep point and write point\_{index}.json.

Particle samples are drawn once (fixed `seed`) so only s varies across points. For Deconvolve, `z_unfolded` is the `z_gen` sample reweighted by `g(z_gen)`; for IBU it is the same sample reweighted by the binned correction `_ibu_point` fits. The subcommand keeps the name `deconvolve` because Deconvolve is what it trains — IBU rides along on the populations that training already required.
...
### `deconvolve.experiments.cubic_sweep::_complete_points`
...
### `deconvolve.experiments.cubic_sweep::collect`
