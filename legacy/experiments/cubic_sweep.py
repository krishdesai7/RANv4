...
from deconvolve.baselines import unfold_variable
...
if TYPE_CHECKING:
    ...
    from deconvolve.coretypes import DatasetSplits, EventArray, VariableOutcome
...
def _ibu_point(pops: Populations) -> tuple[np.double, VariableOutcome]:
    ...
    and is applied to the same `mc.z` that Deconvolve's weights are applied to.
...
def run_ran(
    ...
    # Deferred so that `deconvolve sweep collect`, which only reads JSON and plots,
    ...
    logger.info(
        "s=%.4f  Deconvolve=%.6f  IBU=%.6f  (init seed %d, %d IBU bins)",
        s,
        ran_wd,
        ibu_wd,
        result.seed,
...
def _plot_sweep(
    ...
    _ = ax.plot(s, ran, "o-", label="Deconvolve")
