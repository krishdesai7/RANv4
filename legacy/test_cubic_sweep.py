...
import numpy as np
import pytest
import deconvolve.experiments.cubic_sweep as cs
import deconvolve.train
from deconvolve.experiments.cubic_sweep import (
    _sweep_point,
    collect,
    make_particles,
    response,
...
def test_run_ran_wiring_with_stubbed_training(
    ...
    Real Deconvolve training is cluster work; here train() is stubbed with an instant
    ...
    def fake_train(
        ...
        return TrainResult(
            # TrainResult declares g/d as DeconvolveModel; run_ran only calls g and
            # reads seed, so this test double is sufficient at runtime.
            g=cast("DeconvolveModel", _uniform_weights),
            d=cast("DeconvolveModel", None),
            history={},
            seed=seed or 0,
        )

    # run_ran imports train() lazily from deconvolve.train, so patch it at the source.
    monkeypatch.setattr(deconvolve.train, "train", fake_train)
