...
`ran` has no configuration layer. Every setting is a Typer default hard-coded in
...
- `RAN_CACHE_DIR` is read at import of `rantypes/constants.py:35`.
- `RAN_TIMING` is read at import of `instrumentation/timing.py:122`.
- `RAN_LOG_LEVEL` is read by Typer's `envvar=` at `cli.py:47`.
...
invocation, or wrapping `ran` in a shell alias, or --- as
...
| 2   | Global config  | `$XDG_CONFIG_HOME/ran/ran.toml`, default `~/.config/ran/ran.toml` |
| 3   | Project config | nearest `ran.toml`, or `[tool.ran]` in `pyproject.toml`           |
...
containing either a `ran.toml` or a `pyproject.toml` with a `[tool.ran]` table
wins outright, and the walk stops there. Within that one directory, `ran.toml`
...
```
~/work/          ran.toml            ignored, walk already stopped
  repo/    .git/
    a/          pyproject.toml       [tool.ran]  <- wins, walk stops
      b/        (cwd)
```
...
- **Writing config files.** Discovery is read-only. `ran` never creates a
  skeleton `ran.toml`, and `ran config show` never writes.
...
```toml
# ran.toml --- the same tables, without the `tool.` prefix
[train]
n-epochs = 500
batch-size = 2048
...
[tool.ran.baseline.omnifold]
n-epochs = 80

[tool.ran.uncertainty.collect]
n-bins = 40
```
...
itself, today just `log-level` --- sit at the top level of `[tool.ran]`.

In `ran.toml` the `tool.ran` prefix is dropped: a top-level **key** is a
group-level option and a top-level **table** is a command. `[train]` in
`ran.toml` and `[tool.ran.train]` in `pyproject.toml` denote the same thing.
...
from `ran`, so it stays cheap to import and unit-testable without pulling in
JAX or Keras.
...
(`"ran.toml:/Users/krish/Developer/RANv4/ran.toml"`, `"default"`), which feeds
both `ran config show` and the `_origin` block in `config.json`.

### `src/ran/cli.py` (modified)
...
invocation, not at import. This matters: `rantypes/constants.py:35` already
...
| `RAN_N_EPOCHS=50`              | 50       | `ENVIRONMENT`            |
| `RAN_N_EPOCHS=50 --n-epochs 7` | 7        | `COMMANDLINE`            |
...
- **`--help` reports the effective default.** Inside a project whose
  `ran.toml` sets `n-epochs = 500`, `ran train --help` prints
...
A variance design is a SLURM array of B x S independent `ran uncertainty run`
invocations, all on a shared filesystem, all reading the same working
directory. Layered configuration would let an edit to `ran.toml` at cell 30
...
```
ran uncertainty freeze -d DIR [the same options as run]
```
...
Environment variables are deliberately excluded --- an exported `RAN_*` in a
batch script is as capable of splitting a design as an edited file is.

`ran uncertainty run` with no `design.json` in the directory is an error naming
...
$ ran config show
Layers (nearest last):
  ~/.config/ran/ran.toml                    found
  /Users/krish/Developer/RANv4/ran.toml     found   (walk stopped: git root)
  [tool.ran] in pyproject.toml              shadowed by ran.toml

train.n-epochs      500        ran.toml
train.lr-g          3e-05      default
log-level           debug      env:RAN_LOG_LEVEL

Environment-only (not layered):
  RAN_CACHE_DIR     .cache     unset
  RAN_TIMING        off        unset
```

`ran config show train` scopes the listing to one command. `--json` emits the
...
```json
{
  "n_epochs": 500,
  "_origin": {
    "n_epochs": "ran.toml:/Users/krish/Developer/RANv4/ran.toml",
    "lr_g": "default",
    "batch_size": "env:RAN_BATCH_SIZE"
  }
}
```
...
`training/engine.py:745`. `RAN_TIMING` has the same shape at
