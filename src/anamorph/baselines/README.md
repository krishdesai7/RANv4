...
This directory contains the comparison baselines for the Anamorph project.

Comparison baselines: **IBU** and **OmniFold**

```python
from anamorph.baselines import evaluate_runs  # IBU
from anamorph.baselines.omnifold import evaluate_runs  # OmniFold
```
...
Rebuild the run's dataset and split it into the baseline populations. It provides the run's dataset as populations, at the float64 Anamorph generated it in. Baselines that need another precision call `astype` at their own boundary.
...
IBU (Iterative Bayesian Unfolding) baseline to compare with Anamorph. It is a simple unfolding method that uses a Bayesian approach to unfold the data.

It is implemented in the [**`ibu.py`**](ibu.py) file.
Usage:

```shell
anamorph baseline ibu --run-dir runs/2026-...
anamorph baseline ibu --run-dir runs # all runs
```

IBU performs 1D per-variable unfolding with purity-based automatic binning. It builds the response matrix from MC, unfolds data, and converts the result to per-event weights for evaluation with the same metrics as Anamorph.
...
Run IBU baseline on completed Anamorph runs.
