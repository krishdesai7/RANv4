...
OmniFold needs TensorFlow, which this project cannot depend on directly (see
`docs/claude/omnifold.md`). `_omnifold_worker.py` carries a PEP 723 header;
...
`ran evaluate` uses, so the comparison is scored by shared code rather than a
...
def _worker_env() -> dict[str, str]:
    ...
    The worker's directory (`ran/baselines/`) also contains this module,
...
def _invoke(worker: Path, in_path: Path, out_path: Path) -> None:
    ...
    # `--no-project`: without it uv resolves the script against this
    # project, whose `requires-python` (>=3.14) conflicts with the worker's
    # pinned ==3.13.*.
    command: list[str] = [
        "uv",
        "run",
...
def _record_iteration_timings(result: Mapping[str, NDArray[Any]], /) -> None:
    ...
    `timing`'s tree is one level deep in practice (see `docs/claude/timing.md`),
...
def evaluate_single(
    ...
    # training pass's rows (see docs/claude/timing.md).
    timing.report()
    timing.write(
        run_dir,
