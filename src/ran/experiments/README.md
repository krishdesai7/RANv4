...
Sweep the cubic detector-distortion strength s and compare RAN.

For each s in `linspace(0, 20, n_points)`, apply a deterministic non-linear detector response $r(s, z) = z + s \times z^3$ to fixed particle-level samples $z_\text{truth} \sim \mathcal{N}(0,1)$, $z_\text{gen} \sim \mathcal{N}(-1,1)$, unfold `z_gen` back toward `z_truth` with RAN, and record `Wasserstein(z_truth, z_unfolded)` for each.

RAN runs as a separate subcommand. It writes its own per-point JSON; `collect` joins them on `s_index`. It does not import happens at module scope, so importing this module commits to neither backend.
