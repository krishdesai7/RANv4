...
def _simulate(
    *,
    ...
    a = rng.normal(scale=sigma_a, size=(n_data, 1, n_components))
    b = rng.normal(scale=sigma_b, size=(1, n_init, n_components))
    eps = rng.normal(scale=sigma_eps, size=(n_data, n_init, n_components))
    return 3.0 + a + b + eps
...
class TestDecompose:
    def test_recovers_the_three_components_it_was_simulated_from(self) -> None:
        ...
        components = decompose(grid)

        assert float(components.data[0]) == pytest.approx(0.25, rel=0.15)
        assert float(components.init[0]) == pytest.approx(0.04, rel=0.15)
        assert float(components.interaction[0]) == pytest.approx(0.09, rel=0.05)
    ...
    def test_it_is_elementwise_over_the_trailing_axes(self) -> None:
        ...
        for k in range(5):
            one = decompose(grid[:, :, k])
            assert float(one.data) == pytest.approx(float(components.data[k]))
    ...
    @pytest.mark.parametrize(("n_data", "n_init"), [(1, 8), (8, 1), (1, 1)])
    def test_it_refuses_a_grid_with_a_degenerate_axis(
        self, n_data: int, n_init: int
    ) -> None:
        with pytest.raises(ValueError, match="at least 2 datasets and 2 seeds"):
            decompose(np.zeros((n_data, n_init, 3)))

    def test_it_refuses_a_grid_holding_a_failed_run(self) -> None:
        grid = _simulate(n_data=4, n_init=4, sigma_a=0.1, sigma_b=0.1, sigma_eps=0.1)
        grid[2, 1, 0] = np.nan
...
class TestNaiveQuadrature:
    ...
    def test_it_exceeds_the_total_by_exactly_one_interaction_term(self) -> None:
        ...
        ) == pytest.approx(float(components.interaction[0]))
    ...
    def test_the_two_one_dimensional_sweeps_reproduce_the_overstatement(self) -> None:
        ...
        seed_sweep = float(np.var(grid[0, :, 0], ddof=1))
        data_sweep = float(np.var(grid[:, 0, 0], ddof=1))
        components = decompose(grid)

        assert seed_sweep + data_sweep == pytest.approx(
            float(components.naive_quadrature[0]), rel=0.15
        )
...
class TestComponentCovariances:
    def test_the_diagonals_are_the_scalar_components(self) -> None:
        ...
        covariances = component_covariances(grid)
        components = decompose(grid)
    ...
    def test_it_finds_correlation_that_bin_by_bin_variances_cannot(self) -> None:
        ...
        shared = rng.normal(scale=0.5, size=(60, 1))
        grid = np.stack([shared, -shared], axis=-1) + rng.normal(
            scale=0.02, size=(60, 8, 2)
        )

        corr = correlation(component_covariances(grid).data)

        assert corr[0, 1] == pytest.approx(-1.0, abs=0.05)

    def test_it_refuses_anything_that_is_not_a_grid_of_spectra(self) -> None:
        with pytest.raises(ValueError, match=r"\(B, S, K\)"):
            component_covariances(np.zeros((4, 4, 3, 2)))
    ...
    def test_the_correction_is_not_optional(self) -> None:
        ...
        corrected = component_covariances(grid).data
        raw = np.cov(grid.mean(axis=1), rowvar=False, ddof=1)

        assert np.trace(raw) > 2.0 * np.trace(corrected)
...
class TestCorrelation:
    def test_the_diagonal_is_one(self) -> None:
        ...
        assert np.diag(correlation(cov)) == pytest.approx([1.0, 1.0])
...
class TestBinning:
    def test_quantile_edges_give_roughly_equal_occupancy(self) -> None:
        ...
        column = np.random.default_rng(0).lognormal(size=40_000).astype(np.single)

        counts = np.histogram(column, bins=quantile_edges(column, n_bins=20))[0]
    ...
    def test_a_constant_column_is_refused(self) -> None:
        with pytest.raises(ValueError, match="constant"):
            quantile_edges(np.ones(100, dtype=np.single), n_bins=5)

    def test_binned_spectra_keep_the_design_shape_and_sum_to_one(self) -> None:
        rng = np.random.default_rng(1)
        column = rng.normal(size=5_000).astype(np.single)
        weights = rng.gamma(4.0, size=(3, 4, 5_000))
        ...
        assert spectra.sum(axis=-1) == pytest.approx(np.ones((3, 4)))

    def test_a_weight_vector_of_the_wrong_length_is_refused(self) -> None:
        column = np.zeros(10, dtype=np.single)

        with pytest.raises(ValueError, match="same events"):
            binned_spectra(column, np.ones((2, 2, 9)), edges=np.linspace(-1, 1, 4))
    ...
    def test_the_multinomial_floor_is_the_pure_closure_correlation(self) -> None:
        ...
        assert multinomial_off_diagonal(20) == pytest.approx(-1.0 / 19)
        with pytest.raises(ValueError, match="at least 2 bins"):
            multinomial_off_diagonal(1)


def test_weighted_means_are_the_unfolded_mean_per_run() -> None:
    column = np.array([0.0, 1.0, 2.0], dtype=np.single)
    weights = np.array([[[1.0, 1.0, 1.0], [0.0, 0.0, 3.0]]])

    assert weighted_means(column, weights)[0] == pytest.approx([1.0, 2.0])


def _populations(n: int = 400, seed: int = 0) -> Populations:
    rng = np.random.default_rng(seed)
    z_gen = rng.normal(size=(n, 2)).astype(np.single)
    z_true = rng.normal(size=(n, 2)).astype(np.single)
    return Populations(
        mc=Events(z=z_gen, x=(z_gen + 0.1).astype(np.single)),
        data=(z_true + 0.1).astype(np.single),
        truth=z_true,
    )
...
class TestBootstrap:
    def test_a_replicate_is_the_same_size_as_the_original(self) -> None:
        ...
        replicate = bootstrap(pops, seed=0)
    ...
    def test_it_draws_with_replacement(self) -> None:
        replicate = bootstrap(_populations(), seed=0)

        assert len(np.unique(replicate.mc.z, axis=0)) < len(replicate.mc)

    def test_it_keeps_the_pairing_inside_each_sample(self) -> None:
        """`(z_gen, x_sim)` is one event seen twice, not two draws."""
        pops = _populations()

        replicate = bootstrap(pops, seed=1)

        assert replicate.mc.x == pytest.approx(replicate.mc.z + 0.1)
        assert replicate.data == pytest.approx(replicate.truth + 0.1)

    def test_mc_and_nature_resample_independently(self) -> None:
        """They are two separate samples; coupling them invents a correlation."""
        pops = _populations()

        replicate = bootstrap(pops, seed=2)
        ...
        assert not np.allclose(replicate.mc.z, replicate.truth)

    def test_it_is_reproducible_from_its_seed(self) -> None:
        pops = _populations()

        assert bootstrap(pops, seed=(42, 3)).mc.z == pytest.approx(
            bootstrap(pops, seed=(42, 3)).mc.z
        )
        assert not np.allclose(
            bootstrap(pops, seed=(42, 3)).mc.z, bootstrap(pops, seed=(42, 4)).mc.z
        )
...
class TestEvaluationSet:
    def test_the_held_out_events_are_absent_from_the_training_pool(self) -> None:
        ...
        evaluation = reserve_evaluation_set(pops, n_eval=100, seed=7)

        pool = {row.tobytes() for row in evaluation.pool.mc.z}
        assert all(row.tobytes() not in pool for row in evaluation.z)
    ...
    def test_the_same_seed_reserves_the_same_events(self) -> None:
        ...
        assert reserve_evaluation_set(pops, n_eval=50, seed=42).z == pytest.approx(
            reserve_evaluation_set(pops, n_eval=50, seed=42).z
        )

    @pytest.mark.parametrize("n_eval", [0, 400, 401])
    def test_it_refuses_a_reservation_that_leaves_nothing_to_train_on(
        self, n_eval: int
    ) -> None:
        with pytest.raises(ValueError, match="n_eval must be between"):
            reserve_evaluation_set(_populations(n=400), n_eval=n_eval, seed=0)
...
class TestDesignSpec:
    ...
    def test_an_index_outside_the_grid_is_refused(self, index: int) -> None:
        with pytest.raises(IndexError, match="outside a 4x3 design"):
            DesignSpec(n_datasets=4, n_seeds=3).cell_of_index(index)
...
class TestLoadCells:
    def test_it_stacks_the_grid_in_design_order(self, tmp_path) -> None:
        spec = DesignSpec(n_datasets=2, n_seeds=3)
        for index in range(spec.n_cells):
            _write_cell(tmp_path, spec, index, np.full(5, float(index)))

        design = load_cells(tmp_path, spec)

        assert design.weights.shape == (2, 3, 5)
        assert design.weights[1, 2, 0] == pytest.approx(5.0)
    ...
    def test_the_per_cell_fields_are_stripped_from_the_shared_metadata(
        ...
        for index in range(spec.n_cells):
            _write_cell(tmp_path, spec, index, np.ones(4))

        meta = load_cells(tmp_path, spec).meta

        assert "seed_index" not in meta
        assert meta["variables"] == ["m", "w"]

    def test_an_incomplete_grid_is_refused_by_name(self, tmp_path) -> None:
        """A ragged design would charge the imbalance to the dataset axis."""
        spec = DesignSpec(n_datasets=2, n_seeds=2)
        for index in (0, 1, 3):
            _write_cell(tmp_path, spec, index, np.ones(4))

        with pytest.raises(FileNotFoundError, match=r"missing cells \[2\]"):
            load_cells(tmp_path, spec)

    def test_cells_from_different_designs_are_refused(self, tmp_path) -> None:
        spec = DesignSpec(n_datasets=2, n_seeds=2)
        for index in range(3):
            _write_cell(tmp_path, spec, index, np.ones(4))
        _write_cell(tmp_path, spec, 3, np.ones(6))

        with pytest.raises(ValueError, match="not from one design"):
            load_cells(tmp_path, spec)
...
class TestRunCell:
    ...
    *different* bootstrap replicate, or the dataset axis measures zero.
    ...
    @staticmethod
    def _stub(monkeypatch, seen: list[dict]) -> None:
        import ran.evaluate
        import ran.train
        ...
        monkeypatch.setattr(
            design_module,
            "base_populations",
            lambda *_args, **_kwargs: (_populations(n=600), 2),
        )

        def fake_train(splits, dim, _units, _layers, seed, **kwargs) -> TrainResult:
            seen.append(
                {
                    "seed": seed,
                    "dim": dim,
        ...
        monkeypatch.setattr(
            ran.evaluate,
            "_get_weights",
            lambda _g, z, **_kw: np.ones(len(z), dtype=np.single),
        )
    ...
    def test_every_cell_holds_out_the_same_evaluation_events(
        ...
        spec = DesignSpec(n_datasets=2, n_seeds=2)
        for index in range(spec.n_cells):
            run_cell(index, tmp_path, spec, n_samples=600, n_eval=100)
    ...
    def test_the_dataset_axis_varies_and_the_seed_axis_does_not(
        ...
        assert seen[0]["z"] == pytest.approx(seen[1]["z"])
        assert not np.allclose(seen[0]["z"], seen[2]["z"])
    ...
    def test_the_hyperparameters_reach_training(self, tmp_path, monkeypatch) -> None:
        ...
        assert seen[0]["lr_g"] == pytest.approx(1e-4)
        assert seen[0]["lambda_dispersion"] == pytest.approx(0.02)
...
class TestCollect:
    ...
    @staticmethod
    def _design(tmp_path, monkeypatch, spec: DesignSpec, *, n_eval: int = 300) -> None:
        import ran.uncertainty.report as report_module

        pops = _populations(n=n_eval * 3)
        monkeypatch.setattr(
            report_module, "base_populations", lambda *_a, **_k: (pops, 2)
        )
        rng = np.random.default_rng(0)
        ...
        tilt = rng.normal(size=n_eval)
        alpha = rng.normal(scale=0.30, size=spec.n_datasets)
        beta = rng.normal(scale=0.03, size=spec.n_seeds)
        for index in range(spec.n_cells):
            b, s = spec.cell_of_index(index)
            ...
            # the bootstrap axis.
            weights = np.exp(
                (alpha[b] + beta[s]) * tilt + 0.01 * rng.normal(size=n_eval)
            )
    ...
    def test_it_writes_the_table_the_npz_and_the_figure(
        ...
        summary = collect(tmp_path, spec, n_bins=5)
    ...
    def test_the_recorded_json_carries_the_closure_floor(
        ...
        spec = DesignSpec(n_datasets=4, n_seeds=3)
        self._design(tmp_path, monkeypatch, spec)
        collect(tmp_path, spec, n_bins=5)

        recorded = json.loads((tmp_path / "variance.json").read_text())
    ...
    def test_a_metadata_mismatch_is_caught_rather_than_silently_misaligned(
        ...
        _write_cell(tmp_path, spec, 0, np.ones(50), n_eval=40)

        with pytest.raises(ValueError, match="does not reproduce the design"):
            collect(tmp_path, spec, n_bins=3)
    ...
    def test_it_warns_when_there_are_too_few_replicates_for_the_binning(
        ...
        with caplog.at_level("WARNING"):
            collect(tmp_path, spec, n_bins=8)
